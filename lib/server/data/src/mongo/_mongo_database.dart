...
class _MongoDatabase extends ADatabase {
  ...
  Future<mongo.DbCollection> getUsersCollection() async {
    _checkConnection();

    dynamic output = await con.conn.collection(_USERS_MONGO_COLLECTION);
    await con.conn
        .createIndex(_USERS_MONGO_COLLECTION, key: "id", unique: true);
    return output;
  }
  ...
  static Future<_MongoDatabase> getConnection() async {
    if (_pool == null) {
      _pool = new _MongoDbConnectionPool(_ADataSource.options.getString("mongo"), 3);
    }
